...
import com.digitfellas.ac.customdialog.DatePickerFragment;
...
//                showToast(dayOfMonth + "-" + (month + 1) + "-" + year);
    ...
    public static void loadImagewithProgressBar(final ImageView imageView, final ProgressBar progressBar, String url) {
        ...
        if (url != null && url.length() > 0)
            Picasso.get().load(url).resize(200, 200).centerCrop().error(R.drawable.not_found).into(imageView, new Callback() {
                @Override

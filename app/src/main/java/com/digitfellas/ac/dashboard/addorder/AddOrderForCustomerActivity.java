...
public class AddOrderForCustomerActivity extends BaseActivity implements ProductAdapter.ProductChangeCallBack {
    ...
    private ArrayAdapter<String> customerNameAapter;
    ...
    private void saveAllProduct() {
        ...
        if (productsList != null && productsList.size() > 0 && businessCategoryAdapter.getCount() > 0) {
            ...
            orderRequest.setCustomer((int) customer_id);
            ...
            dataSource.saveCustomerOrder(token, orderRequest, new DataListener() {
                ...
                public void onSuccess(Object object) {
                    ...
                    AddOrderForCustomerActivity.super.onSuccess(object);
...
//                        onBackPressed();
                        ...
                        if (isFinish) {
                            ...
                            Intent intent = new Intent(AddOrderForCustomerActivity.this, DealerDashboardActivity.class);
...
//
                ...
                public void onFail(Throwable throwable) {
                    AddOrderForCustomerActivity.super.onFail(throwable);
                }

                @Override
                public void onNetworkFailure() {
                    AddOrderForCustomerActivity.super.onNetworkFailure();
                }
            });
...
//            Log.e(TAG, "saveAllProduct: " + new Gson().toJson(orderRequest));
            ...
            showToast("There is no product or no business category");
        }
...
//
        ...
        customerNameAapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, new String[]{dataSource.getName() + ""});
...
//                startActivity(intent);
                ...
                if (businessCategoryAdapter.getCount() > 0) {
                    ...
                    if (category != null && mDeliveryDate.getText().length() > 0 && response != null && response.getData() != null) {
                        ...
                        Object o = getBrandByCategoryName(category);
                        if (o != null) {
                            Intent intent = new Intent(AddOrderForCustomerActivity.this, AddProductActivity.class);

                            intent.putExtra("bCategory", (Parcelable) category);

                            intent.putExtra("productandbrand", (Parcelable) o);

                            startActivityForResult(intent, KEY_REQUEST);

                        } else {

                            showToast("There is no product to add");
...
//                setmMessageShow(true);
    ...
    @SuppressLint("LongLogTag")
    private Object getBrandByCategoryName(Category category) {

        String cName = category.getCategoryName();

        if (cName.toLowerCase().equalsIgnoreCase("Mobile".toLowerCase())) {
            Log.e(TAG, "getBrandByCategoryName: " + cName);
            return response.getData().getMobile();

        } else if (cName.toLowerCase().equalsIgnoreCase("ac".toString())) {

            return response.getData().getAc();
        }
    ...
    public void editProduct(int position, Products products) {
        ...
        Intent intent = new Intent(AddOrderForCustomerActivity.this, EditProductActivity.class);
